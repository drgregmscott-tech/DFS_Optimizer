...
def compute_dart_exposure_overrides(players_all: pd.DataFrame, dart_floor_threshold: float,
                                     dart_exposure_cap: float, stack_mode: str,
                                     stack_positions: set, stack_teams: list, stack_games: list,
                                     mini_stack_type: str, candidate_pool_size: int,
                                     diversify_requested: str, bring_back: bool) -> dict:
    """Ad Hoc Session A6 -- --dart-exposure-cap's implementation. Returns
    {player_id: dart_exposure_cap} for every non-DST player whose
    statline_p10 falls below dart_floor_threshold and whose team is not
    this batch's own stack-candidate team(s) (resolved the same way
    build_multi_lineup()'s own stacking will resolve them -- decision
    #33's team-level candidate pool, not a per-lineup dynamic check).
    Deliberately does NOT exempt the bring-back/opponent side -- see
    below.

    Deliberately protects only the stack team itself, NOT its bring-back
    opponent -- found by testing, not assumed. An earlier version of this
    function also exempted each candidate team's real opponent (the
    bring-back side), on the theory that a low floor there is the
    correlation trade you're paying for. Real testing (this same Ad Hoc
    Session A6 chat) showed that's backwards: with --bring-back on, the
    solver satisfies the "one opponent-team player" requirement with
    whichever legal opponent player is cheapest, regardless of that
    player's own floor -- so the bring-back slot can just as easily be
    filled by dead-money filler (this session's real example: Kevin
    Austin Jr., statline_p10 = 0.07, repeatedly chosen as a $3,000 NO
    bring-back purely to free salary for Taylor/Loveland-caliber value
    elsewhere, not because he individually carries any correlation
    upside) as by a real bring-back piece. A genuine bring-back with a
    real floor (this session's example: Michael Wilson, ARI,
    statline_p10 = 2.40) clears this function's floor threshold on his
    own merits regardless of team, so he was never at risk from removing
    this exemption -- only the disguised-filler case was. The QB's own
    stack team stays exempt because that correlation is generated by the
    stack constraint directly (the QB and his teammate need each other to
    produce the same play), not by "cheapest legal name from team X,"
    which is exactly the distinction that made the bring-back side the
    wrong thing to protect."""
    if "statline_p10" not in players_all.columns:
        raise RuntimeError(
            "--dart-exposure-cap requires a projection file built with "
            "--sigma-recalibration (no statline_p10 column in this pool)."
        )

    protected_teams = set()
    if stack_mode != "none":
        candidates, _, _ = resolve_stack_candidates(
            players_all, stack_mode, stack_positions, stack_teams, stack_games,
            mini_stack_type, candidate_pool_size, diversify_requested,
            bring_back=bring_back,
        )
        for cand in candidates:
            if cand.get("target_team"):
                protected_teams.add(cand["target_team"])
            if cand.get("target_game"):
                protected_teams.update(cand["target_game"])
        # Deliberately NOT extended to each protected team's real
        # opponent (the bring-back side) -- see this function's own
        # docstring for why that turned out to be the wrong exemption.

    is_dst = players_all["position"].astype(str).str.upper().isin(DEFENSE_POSITION_LABELS)
    p10 = pd.to_numeric(players_all["statline_p10"], errors="coerce")
    dart_mask = (
        (~is_dst)
        & (p10 < dart_floor_threshold)
        & (~players_all["team"].isin(protected_teams))
    )
    dart_ids = set(players_all.loc[dart_mask, "player_id"])
    return {pid: dart_exposure_cap for pid in dart_ids}
...
def main():
    ...
    # Ad Hoc Session A6 (2026-09-13) -- Dart Exposure Cap. Grew out of a
    # user chat walking a real DK MME (20-lineup) build: a hand-inspection
    # of the exposure printout found Kevin Austin Jr. (a $3,000 WR3/bring-
    # back filler, statline_p10 = 0.07 -- essentially a coin-flip on
    # scoring anything at all) sitting at 20% exposure purely because he
    # was the cheapest legal filler in several lineups, not because of any
    # real correlation reasoning. Manually re-running with
    # --player-exposure capped him correctly, but that required first
    # eyeballing the exposure table and hand-picking the offending
    # player_id -- these two flags automate that same judgment call.
    #
    # Deliberately excludes DST: `statline_p10` is a flat 0.0 for every
    # DST regardless of matchup quality (the DST simulation doesn't
    # produce real percentile bands -- see the `sigma`/`statline_p10`
    # columns for any DEF/DST row), so a floor-based filter would
    # nonsensically catch the *best* matchup play (e.g. a team facing the
    # slate's worst offense) exactly as hard as the worst one. DST
    # exposure should be earned or lost on the opponent's own implied
    # team total, a completely different, already-existing signal
    # (`vegas_factor`/`implied_total`) -- not this flag's business.
    #
    # Also deliberately exempts this batch's own stack candidate team(s)
    # ONLY, not their real opponent(s) this week (the bring-back side) --
    # tested both ways in the same chat that motivated this flag. Exempting
    # the bring-back side too seemed right at first (a low floor there
    # looks like the correlation trade you're paying for) but real testing
    # showed the opposite: with --bring-back on, the solver satisfies "one
    # opponent-team player" with whichever legal name is CHEAPEST,
    # regardless of that player's own floor -- so the bring-back slot is
    # just as often filled by the exact dead-money filler this flag exists
    # to catch (this session's real example: Kevin Austin Jr., a $3,000 NO
    # WR with statline_p10 = 0.07, repeatedly chosen as Detroit's bring-
    # back purely to save cap space, not for his own merits). A genuine
    # bring-back with a real floor (this session's example: Michael
    # Wilson, ARI, statline_p10 = 2.40, Herbert's LAC/ARI bring-back)
    # clears the floor threshold on his own merits and was never at risk
    # from removing this exemption. The QB's own stack team stays exempt
    # because THAT correlation is generated directly by the stack
    # constraint (QB and teammate need each other to produce the same
    # play), which is the distinction the bring-back side lacked.
    #
    # No backtest validates a specific cap % or floor threshold the way
    # Session 10.5b's lambda sweep does for `--lambda` -- these are a
    # heuristic guard against a concrete failure mode observed in a real
    # build, not a swept/optimized value. Treat the presets' 0.10/1.0
    # defaults as a reasonable starting point to revisit, not a validated
    # constant.
    parser.add_argument(
        "--dart-floor-threshold", type=float, default=pdef("dart-floor-threshold", 1.0),
        help="A non-DST player whose statline_p10 (10th-percentile "
             "simulated outcome) falls below this value is considered a "
             "'dart' for --dart-exposure-cap's purposes (default 1.0 -- "
             "roughly 'better than even odds of scoring next to nothing'). "
             "Has no effect unless --dart-exposure-cap is also set. "
             "Requires a projection file built with --sigma-recalibration "
             "(same requirement as --lambda) -- fails loudly if the pool "
             "has no per-player statline_p10.",
    )
    parser.add_argument(
        "--dart-exposure-cap", type=float, default=pdef("dart-exposure-cap", None),
        help="Caps every 'dart' player (see --dart-floor-threshold) at "
             "this fraction of your --n-lineups batch (same 0.0-1.0 "
             "convention as --max-exposure/--player-exposure), UNLESS that "
             "player is on this batch's own stack-candidate team(s) -- a "
             "low floor there is the direct QB-stack correlation trade "
             "you're paying for, not accidental dead-money risk. The "
             "bring-back/opponent side is deliberately NOT exempt: testing "
             "showed a --bring-back build satisfies its 'one opponent-team "
             "player' requirement with whichever legal name is cheapest "
             "regardless of that player's own floor, so a real bring-back "
             "with a real floor clears this cap on his own merits anyway, "
             "while a disguised dead-money filler (the exact case this "
             "flag exists for) does not get a free pass just for sitting "
             "on the correlated team. DST is always exempt (statline_p10 "
             "is a flat 0.0 for every DST regardless of matchup, so a "
             "floor filter can't tell a good matchup from a bad one there "
             "-- see this flag's block comment above). An explicit "
             "--player-exposure entry for a given player_id "
             "always wins over this flag's computed cap for that player -- "
             "this only fills in players you haven't already named "
             "yourself. Only used with --n-lineups; classic slates only "
             "for now (no effect on Showdown). Omit (default) for "
             "unchanged prior behavior -- every dart-tier player still "
             "only bound by --max-exposure or an explicit --player-"
             "exposure entry, same as before this flag existed.",
    )
    parser.add_argument(
    ...
    # rationale (DST exemption, team-level-not-per-lineup stack/bring-back
    # exemption). An explicit --player-exposure entry for a player_id
    # always wins over this flag's computed cap for that same id -- this
    # only fills in gaps the user hasn't already named themselves.
    if args.dart_exposure_cap is not None:
        if showdown_mode:
            parser.error(
                "--dart-exposure-cap is not supported for Showdown slates "
                "yet (classic only -- Ad Hoc Session A6)."
            )
        if not 0.0 <= args.dart_exposure_cap <= 1.0:
            parser.error("--dart-exposure-cap must be between 0.0 and 1.0.")
        if not args.n_lineups:
            print(
                "NOTE: --dart-exposure-cap was set but --n-lineups was not "
                "-- exposure has no meaning for a single lineup, ignored "
                "for this build.", file=sys.stderr,
            )
        else:
            dart_overrides = compute_dart_exposure_overrides(
                _format_probe, args.dart_floor_threshold, args.dart_exposure_cap,
                args.stack_mode, stack_positions, stack_teams, stack_games,
                args.mini_stack_type, args.stack_candidate_pool,
                args.stack_diversify, args.bring_back,
            )
            player_exposure = player_exposure or {}
            newly_capped = {
                pid: pct for pid, pct in dart_overrides.items()
                if pid not in player_exposure
            }
            player_exposure.update(newly_capped)
            if newly_capped:
                # Most of these are bench/inactive-caliber names the solver
                # was never going to roster anyway (this cap is a harmless
                # no-op for them) -- naming all of them would be noise, not
                # signal, so only list names when the list is short enough
                # to actually be useful; otherwise just the count.
                if len(newly_capped) <= 15:
                    name_lookup = _format_probe.set_index("player_id")["player_name"]
                    capped_names = sorted(name_lookup.get(pid, pid) for pid in newly_capped)
                    detail = ": " + ", ".join(capped_names)
                else:
                    detail = " (mostly bench-caliber names never in contention -- see per-player exposure in the run's own output for who it actually affected)"
                print(
                    f"Dart exposure cap: {len(newly_capped)} player(s) with "
                    f"statline_p10 < {args.dart_floor_threshold} capped at "
                    f"{args.dart_exposure_cap:.0%} (DST and this batch's own "
                    f"stack team(s) exempt, bring-back side NOT exempt){detail}"
                )
