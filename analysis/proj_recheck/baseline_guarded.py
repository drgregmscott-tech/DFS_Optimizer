"""GUARDED re-baseline (Step A2, 2026-09-24): same harness as accuracy.py, FIX/FIX_NS now read from the
guarded, leak-free rebuilds (--backtest-no-leak for wk2; wk1 season 2025 week 23) in the dir given as argv[1].
Outputs are written with a guarded_ prefix.

Original doc -- Projection accuracy recheck: OLD (committed pre-fix output/) vs FIXED no-stack (engine) vs FIXED (+stack).
...
def frame():
    ...
    for sid in SL:
        ...
        f = pd.read_csv(S / name, dtype={"player_id": str})
...
if __name__ == "__main__":
    ...
    D.to_csv(OUT / "guarded_accuracy_frame.csv", index=False)
    ...
    A.to_csv(OUT / "guarded_accuracy_summary.csv", index=False)
    ...
    R = rankq(D)
    R.to_csv(OUT / "guarded_ranking_by_slate_pos.csv", index=False)
