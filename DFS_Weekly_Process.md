...
**Only the ownership-independent method is live right now** (`worst_top25_realstack`). **Corrected 2026-09-23:** its original "3/6 cash, 0.703 mean percentile" was measured on pre-projection-fix projections; re-scored on the fixed projections it is 1/6, 0.583 (n=6 — not statistically distinguishable from the alternatives, but not a demonstrated edge either; see `HANDOFF_week3_lineup_system.md`). The stronger chalk-anchor + ownership-driven-pivot method's "4/6, 0.819 with real ownership" was also overstated: it used real ownership that was missing all FLEX rows, and corrected it is 2-3/6, ~0.72-0.75 — and three live-substitution tests (feeding it the model's own pre-lock ownership estimate) still found the pivot layer added zero value, so it stays out. Don't expect to see any leverage/pivot swaps in this output — a plain single lineup with no differentiation notes is the current expected shape, not missing functionality.
