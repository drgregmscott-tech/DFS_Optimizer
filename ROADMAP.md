...
**⚠️ Note added 2026-08-11, updated 2026-09-14:** this session was never formally run in its scheduled Aug 13-15 preseason window -- project attention moved into Phase 10-14 engine work instead, and no `/logs/dry_run_week1_issues.md` punch list exists. Its core intent -- run the full pipeline live, both sites, against real data, for the first time -- was substantively achieved for FD by **Session 14.1c (2026-08-11)**, run against a real regular-season Week 1 2026 slate instead of preseason. That session found and fixed five real bugs live (see its SESSION_LOG.md entry), which is exactly the kind of outcome this card exists to produce. The one item not carried over from 14.1c -- **the cron-job.org near-lock automation firing during a real lock window -- is now also closed** (2026-09-14), via real Week 1 2026 regular-season lock windows; see the validation checklist below for the real evidence. **This card is now closed in substance:** both DK and FD real pipeline mechanics, and the real near-lock automation, have all been exercised against real lock windows -- just via the regular season rather than the originally-scheduled preseason rehearsal.
...
**Validation:**
- [ ] Full pipeline runs start to finish without manual intervention, for both sites (the actual goal of this test) — moot as scoped (preseason-specific), see Session 6.2/6.3's notes; substantively covered by real Week 1 2026 runs instead.
- [ ] Every failure/manual fix needed is logged, tagged by site — no formal punch list was ever kept in this exact format; real issues found along the way are scattered across Ad Hoc Sessions A1-A7 and Session 14.1c instead.
- [ ] Generated lineups' actual results vs projections compared, both sites (expect roughness in preseason — focus on pipeline reliability over accuracy) — superseded by Session 9.1's real regular-season comparison (all six Week 1 classic slates, both sites).
- [x] **Confirm the near-lock automation (cron-job.org → Cloudflare Worker → GitHub Actions) actually fires during a real lock window and produces a real, correctly-timed refresh — first genuine real-world exercise of Session 5.2's mechanism.** ✅ **Closed 2026-09-14**, via real Week 1 2026 regular-season lock windows rather than a preseason one. `logs/automation_run_log.csv` shows real `full_refresh_dispatch` events (the `repository_dispatch`/`near_lock_refresh` path — i.e. the actual cron-job.org → Worker → GitHub Actions chain, not the separate native-cron `full_refresh_scheduled` path) firing and succeeding close to both real lock windows on 2026-09-13: three dispatches at 15:01/16:01/16:31 UTC ahead of the 17:00 UTC main/early lock, and one at 19:31 UTC ahead of the 20:00 UTC afternoon lock. This also closes the matching "Session 5.2's real weekly cron-job.org schedule + `current_slate.json`" entry in Known Deferred Validations below — same real-world event, same evidence.

**The remaining three checkboxes above are not being separately re-run** — this card's real underlying goal (prove the pipeline and the near-lock automation work against a real slate before Regular Season Week 1) was met via the actual regular season itself, not a preseason rehearsal. Treat this card as closed by substitution, not by literal completion of its own scope.

**Handoff notes to log:** the full punch list, prioritized, with each item tagged DK/FD/both. Never produced in this format — see Ad Hoc Sessions A1-A7 and Session 14.1c's own logs instead for the real issues found and fixed along the way.
...
**QB error deep-dive (2026-09-14) — investigated, not a new code bug, see SESSION_LOG.md's "QB Bonus-Integration Investigation" entry for the full trace.** DK's QB mean error (+5.80-6.08 depending on slate) was initially suspected to be a Monte-Carlo bonus-integration defect (DK's step-function yardage bonuses need per-draw scoring, per `scoring_rules.py` decision #3). Traced and **ruled out**: `statline_model.simulate()` correctly calls `scoring_rules.score_statline()` per individual Monte-Carlo draw and averages afterward, exactly as required. The real pattern: DK QBs who crossed a real bonus threshold (300 pass yd or 100 rush yd — 5/28 on the main slate) averaged **+18.4** error vs. +3.4 for the rest; the same bonus-eligible players *also* showed a large error on FD (+16.1) despite FD paying no such bonus at all, confirming the underlying miss is shared market/volume variance (Vegas's own real pregame implied totals for these specific teams — Buffalo 22.81, Green Bay 22.22, Jacksonville 24.42 — were genuinely modest, and each team significantly outscored its own line that week), not a DK-specific defect. DK's bonus structure mechanically amplifies whatever a given week's Vegas-total miss turns out to be — a real, correctly-modeled structural fact about DK's rules, not something to fix. One real week / 5 bonus-hit QBs is too small a sample to separate "real signal" from "this week's Vegas lines happened to miss" — worth re-checking this same bonus-hit-vs-not split after a few more real weeks are logged, as a purely observational check (not gated on any code change).
...
- ~~**Session 5.2's real weekly cron-job.org schedule + `current_slate.json`, both sites.**~~ ✅ **RESOLVED (2026-09-14).** Both were hand-updated to real Week 1 2026 values ahead of the real lock, and the near-lock `repository_dispatch` chain fired for real against both real lock windows (`logs/automation_run_log.csv`: three dispatches ahead of the 17:00 UTC main/early lock on 2026-09-13, one ahead of the 20:00 UTC afternoon lock) — see Session 6.1's card above for the full evidence. This needs redoing every week going forward (new slate/lock times, same mechanism) — that's normal weekly process, not a re-opening of this gap.
...
- ~~**Session 7.3's Game Stack QB requirement (decision #30) and multi-team/multi-game stack pinning (decision #32), full-size slate.**~~ ✅ **RESOLVED, both sites.** FD: Session 14.1c (2026-08-11) — real 12-game/24-team Week 1 2026 slate, stack build confirmed live; user's explicit call that this size is a good enough test of full-slate stack feasibility. **DK: RESOLVED (2026-09-14)** — the real DK Week 1 main slate alone spans 24 teams (well beyond the original 6-team Madden Sim pool this item was worried about), and Ad Hoc Session A3's real click-through builds ran the `se_gpp`/`mme_gpp` presets (`--stack-mode qb --bring-back`) against it live, both sites, "everything passed with flying colors" per that card. Same real-slate-size bar FD was already held to.
...
- [x] Root cause of the Sep-3 CI gap documented in SESSION_LOG.md. **Closed 2026-09-14** — see SESSION_LOG.md's "Ad Hoc Session A1 (retroactive write-up)" entry (the finding itself dates to 2026-09-10; only the write-up was missing).
...
- [ ] Session 11.1's own validation checklist (holdout MAE improves over baseline, budget constraint still holds, top-5-owned rank ordering correct) — see that card above, unchanged. **Blocked, progress underway:** A2 step 1 has run (1 of 4-6 real DK regular-season weeks logged as of 2026-09-14 — see A2's card) — the zero-rows blocker is gone, but 3-5 more real weeks are still needed before this can begin.
...
- [x] Frontend UI wired up same-day, on request: a "Dart Exposure Cap" checkbox + Cap %/Floor Threshold sub-fields in `dfs_optimizer_frontend/index.html`'s Build A Lineup panel (on by default in the built-in "MME GPP (20-150)" preset, matching `data/optimizer_presets.json`), `dart_exposure_cap`/`dart_floor_threshold` added to `cloudflare_worker/optimizer_api/optimizer_api.js`'s passthrough allowlist, and two new `flag()` lines in `.github/workflows/run_optimizer_dispatch.yml`'s arg-builder. Validated: inline JS syntax-checks clean, every `getElementById` resolves to a real id (no dangling refs either direction), YAML + embedded Python block both parse/compile clean, and a simulated real dispatch payload run through the actual arg-builder logic produced the exact right `optimizer.py` CLI string. `optimizer_api.js` deployed live via `npx wrangler deploy` (Worker isn't Git-integrated, unlike the Pages frontend — confirmed this distinction with Greg before deploying, since a Worker only updates on an explicit deploy, not on `git push`). Not yet done: Greg's own live click-through confirmation on the deployed site (same standing gap Session 16 flagged for its own two features).

---

### Ad Hoc Session A7 — Exposure Summary Table (frontend)
**Status:** ✅ Complete (2026-09-13)

**Trigger:** immediately after Ad Hoc Session A6 shipped, Greg asked for a way to actually SEE each player/DST's real exposure across a built batch without digging through the GitHub Actions log — specifically to confirm chalk plays land near Max Exposure % and dart-tier plays land near the new Dart Exposure Cap %, and to spot anything unexpectedly over/under-exposed relative to his own read of the slate.

**What was built:** a new "Exposure Summary" table in `dfs_optimizer_frontend/index.html`, placed directly below the Player Pool section (same tabbed ALL/QB/RB/WR/TE/DST/K pattern, kept as its own independent tab state so filtering one table never affects the other). Purely client-side — computed from `state.data[site]` (the same multi-lineup data structure the existing single-lineup nav viewer already holds, whether it arrived via a live "Build Lineups" run or a manually-uploaded `lineups_multi_*.csv`), so no backend/CLI/Worker changes were needed at all. Tallies each player's appearance count across every lineup in the batch, shows Team/Player/Pos/Salary/Proj/Used (e.g. "7/20")/Exposure %, sorted highest-exposure-first. Hooked into the existing `render()` function (called unconditionally at its top, before the existing early-return-if-no-data check) so it refreshes automatically after every build, site switch, or slate load with zero new event wiring beyond the table's own position-tab clicks.

**Validation:**
- [x] Inline JS syntax-checks clean (`node --check`); every `getElementById` reference resolves to a real element id, no dangling references either direction.
- [x] Real functional test, not just a syntax check: opened the live page, synthesized a 5-lineup CSV by hand with known, deliberately-designed overlaps (a player in 5/5 lineups, one in 4/5, one in 3/5, five different players each in exactly 1/5), dispatched it through the page's own real upload path (a real `File`/`DataTransfer` object into the existing file input, not a shortcut — exercises the exact same code path a real user's upload does), and confirmed every single computed percentage (100%/100%/80%/60%/20%×5) and the sort order (highest exposure first, ties broken by projection) matched the hand-designed expected values exactly.
- [x] Position-tab filtering confirmed both ways: an isolated "RB" click correctly showed only the two RB rows; clicking "DST" without first resetting to ALL correctly showed the union of the still-selected RB tab plus DST (confirmed this was correct multi-select behavior matching the existing Player Pool tabs, not a bug, before concluding anything).
- [x] Empty-state message ("No lineups built yet…") confirmed showing correctly before any lineup data exists for the active site.
