...
- **GUARD IMPLEMENTED 2026-09-24 (scripts/statline_model.py; ownership session, Greg approved):** (1) a depth-chart QB1 who changed teams now counts toward `raw_sum` in `reconcile_team_shares()` (new `depth_qb1` flag set in `apply_volume_prior`); (2) a QB with NO depth-chart entry on a team whose chart names a QB1 is now suppressed like a listed non-#1 (wk3 MIN: unlisted Brosmer was taking 16 att); (3) backstop `QB_PASS_ATT_CAP = 55` clamps + warns. Validated: wk1 Cousins/Murray/Geno/Willis 62/57/52/33 pts -> 13.0/14.1/11.5/14.7 (~31 att); wk1 QB(>8 pts) vs actual, old->guard: bias -7.0/-8.2/-3.5 -> -2.7/-4.6/-0.5, RMSE 12.1/13.1/9.2 -> 9.6/10.4/7.1, corr .19/.14/.20 -> .37/.35/.23 (main/early/afternoon); wk2 main unchanged (no blow-ups, no regression); wk3 MIN Murray 9.5 -> 12.7 pts / 13.7 -> 30.2 att, backups -> ~1 pt. Side effects to eyeball: 41 wk3 backup QBs drop to ~1 pt and 84 non-QB players shift slightly via team pass volume. STILL TO DO here: rebuild slates + refit ownership after the rest of the projections work; watch that MIN pass `scale` is 6.0x (Murray own mu ~5 -> team target) -- fine mechanically but a sign his own mu is low from thin history.
- **Original recommended guard (now done, see above):** scale only players counted in `raw_sum` (or cap a moved starter's volume / cap `scale`, e.g. <=~1.5), plus a fail-loud check on any player projecting pass attempts > ~50. Validate on the wk1 cases above AND on the live wk3 MIN case below.
